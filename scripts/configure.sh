#!/usr/bin/env bash
# Interactive setup wizard for FinHub.
...
printf "\n${BOLD}${CYAN}FinHub Configuration Wizard${NC}\n"
...
case $sandbox in
    ...
    2)
        ...
        namespace_default="finhub-local-$(printf '%s' "$(hostname)-$REPO_ROOT" | cksum | awk '{print $1}')"
...
if [ -z "$_BYOK" ] || [ "$_BYOK" = "finhub-local-dev-encryption-key" ]; then
