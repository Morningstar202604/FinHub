...
# Intervals served by each family. Stock/index route through the finhub-data
...
async def _fetch_stock_series(
    ...
    """Fetch stock OHLCV: finhub-data (US) → FMP fallback (global).
    ...
    # finhub-data first (US equities only; returns None for anything else).
    finhub_result = await _finhub.fetch_stock_data(legacy, canonical, start_date, end_date)
    if isinstance(finhub_result, dict):
        return None, error_from_upstream(
            finhub_result.get("error", "finhub-data request failed"),
            symbol=display,
            interval=canonical,
        )
    if finhub_result is not None:
        # Already-normalized display rows, descending → flip to ascending.
        data = list(reversed(finhub_result))
        return {
            "data": data,
            "source": "finhub-data",
            "symbol": display,
...
async def get_asset_data(
    ...
    # commodity / crypto / forex — FMP direct (the provider chain and finhub
...
async def get_short_data(
    ...
    off-exchange (date). finhub-data only.
    ...
    if not await _finhub.ensure():
        return make_error(
            "client_unavailable",
            "Short data requires finhub-data (not configured).",
            symbol=display,
        )

    raw = await _finhub.fetch_short_data(
