...
- finhub-data only (no FMP fallback)
...
from data_client.finhub_data import (
...
_finhub = get_finhub_mcp_client()

# The finhub-data option aggregates endpoint serves the full canonical vocab.
_OPTION_INTERVALS = {
...
def _finhub_error(result: dict, **context: Any) -> dict:
    """Contract error envelope from a finhub-data client error dict."""
    return error_from_upstream(
        result.get("error", "finhub-data request failed"), **context
    )
...
async def get_options_chain(
    ...
    Filter by call/put, expiration range, and strike range. finhub-data only.
    ...
    if not await _finhub.ensure():
        return make_error(
            "client_unavailable",
            "Options data requires finhub-data (not configured).",
            symbol=display,
        )

    result = await _finhub.fetch_options_chain(
        underlying_ticker,
    ...
    if "error" in result:
        return _finhub_error(result, symbol=display)
...
async def get_options_prices(
    ...
    "1d"/"daily" accepted). finhub-data only.
    ...
    result = await _finhub.fetch_options_prices(
        opt, from_date=from_date, to_date=to_date, interval=canonical,
    )
    if isinstance(result, dict):
        return _finhub_error(result, symbol=opt, interval=canonical)
...
async def get_options_snapshot(
    ...
    result = await _finhub.fetch_options_snapshot(tickers)
    if "error" in result:
        return _finhub_error(result, tickers=options_tickers)
