...
8. **Research Loop Kickoff（研究闭环起点）** - For the first workspace, kick off the `research-loop` mainline so the user sees the product's core value immediately:
   - Create the workspace with a description that frames it as a research loop (e.g. "研究闭环：跟踪我关注的标的，建立观点→数据→模型→报告→跟踪").
   - Instruct the workspace agent to bootstrap its `agent.md` with the research-loop structure: 目标、当前阶段（从阶段①选题观点开始）、关键发现、文件索引、闭环历史。
   - Dispatch the starter question so it enters 阶段①（选题/观点）：ask about one holding or watchlist item the user cares most about, and have the agent produce a falsifiable thesis + evidence snapshot, then offer to set up follow-up tracking (cron or price trigger). This turns a one-off Q&A into an ongoing loop.
   - If the user has portfolio holdings, the starter analysis must reference the relevant `portfolio.json` / `watchlist.json` entry so the conclusion is portfolio-linked from day one.
