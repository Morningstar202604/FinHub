...
Template:
```bash
gh issue create --repo "finhub/FinHub" \
...
Steps:
1. Clone or update: if `.self-improve/finhub` exists, `cd .self-improve/finhub && git checkout main && git pull origin main` to get latest. Otherwise `gh repo clone "finhub/FinHub" .self-improve/finhub -- --depth 1`
2. Branch: `cd .self-improve/finhub && git checkout main && git checkout -b bot/fix/<short-desc>`
...
6. PR:
```bash
gh pr create --repo "finhub/FinHub" \
  --base main \
...
- Clone to `.self-improve/finhub` (inside workspace, persists across restarts)
...
- [ ] **Correct repo** — targeting `finhub/FinHub`
