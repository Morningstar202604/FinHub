"""Pagination + sort behavior for the finhub-data aggregates client.
...
from src.data_client.finhub_data.client import FinHubDataClient
...
async def _client_with(pages: list[dict[str, Any]]) -> tuple[FinHubDataClient, _FakeTransport]:
    transport = _FakeTransport(pages)
    client = FinHubDataClient(base_url="http://finhub-data.test")
    # Swap the HTTP layer for our scripted transport.
    client.http = httpx.AsyncClient(
        base_url="http://finhub-data.test",
        transport=transport,
        timeout=1.0,
    )
...
async def test_default_sort_is_desc():
    ...
    # Sent as query param to finhub-data
