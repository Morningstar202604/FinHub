...
Requires:  FMP_API_KEY (for OHLCV), FINHUB_DATA_URL (for short data)
"""
...
_has_finhub = bool(os.getenv("FINHUB_DATA_URL"))
...
async def _init_finhub():
    """Ensure the finhub-data MCP client is initialized for the current event loop.

    The module-level ``_finhub`` is a :class:`FinHubMCPClient` whose HTTP
    ...
    if not await client.ensure():
        ...
        url = os.getenv("FINHUB_DATA_URL", "")
        token = os.getenv("INTERNAL_SERVICE_TOKEN", "")
        if not url:
            pytest.skip("FINHUB_DATA_URL not set")
...
@pytest.mark.skipif(not _has_finhub, reason="FINHUB_DATA_URL not set")
class TestGetShortDataLive:
    async def test_both(self):
        mod = await _init_finhub()
        result = await mod.get_short_data("AAPL")
        assert "error" not in result, result.get("error")
        assert result["source"] == "finhub-data"
