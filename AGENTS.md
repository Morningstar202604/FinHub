...
- **The full `pnpm test` run has a rotating flake set — measure before you chase it.** Across repeated runs of the same tree the failure *count* and *membership* both change (observed 4 → 2 → 4 failures, drawing from `AddWidgetDialog`, `brokerageSurface`, `ExportPreviewModal`, and others), while every named file passes in isolation. 303 files at default parallelism is the cause, not the code. So: never conclude a regression from a single full-suite run. Re-run the named files alone first — if they pass, it was contention, and the honest next step is to leave them alone rather than "fix" a test that was never broken. Only a failure that reproduces in isolation is a real one.
