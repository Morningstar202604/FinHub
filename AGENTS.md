...
- **Subagents** (`agent/subagents/`): ten built-ins (`research`, `general-purpose`, `data-prep`, `equity-analyst`, `report-builder`, plus the finance-department roster `accountant` / `treasury` / `tax-specialist` / `fp-analyst` / `internal-auditor`), all enabled by default; user definitions in `agent_config.yaml` override any of them by name.
- **Finance committee** (`workflows/finance_committee/`): the first shipped prebuilt RunWorkflow — a six-phase meeting (agenda → statements → cross-examination → risk gate → minutes → independent verification) that the lead agent convenes via `RunWorkflow(workflow="finance_committee")`. Design, role templates, and contract red lines in `docs/FINANCE_COMMITTEE.md`; its dispatch contract is locked offline by the `finance_committee` evals suite (`uv run python -m evals run finance_committee`).
...
- **Deployment**: `docker-compose.yml` / `docker-compose.prod.yml`; Dockerfiles in `deploy/` + root `Dockerfile.sandbox`; `make deploy` / `make prod-up`. Prebuilt workflows ship from `workflows/` — the backend Dockerfiles `COPY` it (registry is cwd-anchored at `/app` in containers) and `docker-compose.yml` mounts `./workflows:/app/workflows` for hot reload; add the mount + COPY to any new deployment surface before it claims to run the committee.
