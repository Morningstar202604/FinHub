...
def upgrade() -> None:
    ...
    """)
    # (user_id, plugin_id) as a pair would require the upstream's
    # "ON DELETE SET NULL (plugin_id)" — a column-list SET NULL, which
    # PostgreSQL does not support (SET NULL always nulls every FK column, and
    # user_id is NOT NULL). The upstream migration never runs on a stock PG.
    # user_plugin_id is a globally-unique UUID primary key, so a single-column
    # FK on plugin_id with ON DELETE SET NULL gives the exact intended
    # semantics: uninstalling a plugin nulls only plugin_id and the secret
    # survives. Same-user pairing is redundant here — user_plugin_id uniquely
    # identifies one plugin of one user, and every writer scopes by user.
    op.execute("""
        ALTER TABLE user_vault_secrets
        ADD CONSTRAINT fk_user_vault_secrets_plugin
        FOREIGN KEY (plugin_id)
        REFERENCES user_plugins(user_plugin_id)
        ON DELETE SET NULL
    """)
