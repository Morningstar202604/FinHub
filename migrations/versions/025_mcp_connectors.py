...
def upgrade() -> None:
    # sandbox_egress_grants originally used UNIQUE NULLS NOT DISTINCT, which
    # requires PostgreSQL 15+. Local deployments on PostgreSQL 14 need the
    # PG14-compatible equivalent: two partial unique indexes that preserve the
    # exact same uniqueness semantics (see below).
    op.execute("""
    ...
    op.execute("""
        CREATE TABLE IF NOT EXISTS sandbox_egress_grants (
            ...
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
            -- connection_id is NULL for any non-'oauth_mcp' kind. The upstream
            -- migration used UNIQUE NULLS NOT DISTINCT (workspace_id, kind,
            -- connection_id) (PG15+); on PG14 the identical semantics are
            -- enforced by the two partial unique indexes below.
        )
    """)
    # PG14-compatible replacement for UNIQUE NULLS NOT DISTINCT: rows with a
    # connection_id are unique per (workspace, kind, connection_id); rows with
    # NULL connection_id are unique per (workspace, kind).
    op.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS uq_sandbox_egress_grants_ws_kind_conn
        ON sandbox_egress_grants(workspace_id, kind, connection_id)
        WHERE connection_id IS NOT NULL
    """)
    op.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS uq_sandbox_egress_grants_ws_kind_null
        ON sandbox_egress_grants(workspace_id, kind)
