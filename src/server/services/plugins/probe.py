...
async def probe_streamable_http(key: str, url: str) -> ProbeResult:
    ...
    try:
        # Inside the timeout, not before it: pinning resolves DNS through an
        # unbounded getaddrinfo, and this coroutine is holding a concurrency
        # slot the moment it starts. A stalling resolver outside the deadline
        # is the one way the phase overruns the bound the caps promise.
        ...
        # str() on a TimeoutError is empty, and the broad handler below would
        # report the one failure the caps exist to produce as no reason at all.
        return ProbeResult(
            key=key, url=url, ok=False,
            detail=f"no response within {PROBE_TIMEOUT_S}s",
        )
