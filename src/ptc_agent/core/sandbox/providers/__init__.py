...
def create_provider(config) -> SandboxProvider:
    ...
    if provider_name == "memory":
        # Local in-process provider: no container/cloud required. Runs sandbox
        # commands as plain subprocesses on the host — do NOT use for untrusted
        # workloads. Useful for local dev and locked-down environments.
        from ptc_agent.core.sandbox.providers.memory import MemoryProvider
