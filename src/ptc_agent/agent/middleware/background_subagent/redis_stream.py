...
async def stamp_task_retention(
    ...
    *,
    ...
    from src.utils.cache.redis_cache import get_cache_client, redis_supports_expire_nx

    cache = get_cache_client()
    if not getattr(cache, "enabled", False) or not cache.client:
        return
    ttl = get_redis_ttl_workflow_events()
    use_nx = await redis_supports_expire_nx(cache.client)
    async with cache.client.pipeline(transaction=False) as pipe:
        if use_nx:
            ...
            # Redis 6 has no EXPIRE NX; plain expire still starts/extends the
            # attach-grace window, which is the best-effort degradation.
            pipe.expire(task_stream_key(thread_id, task_id), ttl)
            pipe.expire(task_meta_key(thread_id, task_id), ttl)
            if task_run_id:
                pipe.expire(run_stream_key(thread_id, task_run_id), ttl)
