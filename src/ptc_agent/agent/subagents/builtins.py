...
BUILTIN_SUBAGENTS: dict[str, SubagentDefinition] = {
    ...
    "accountant": SubagentDefinition(
        name="accountant",
        description=(
            "Delegate accounting work to the accountant sub-agent: journal "
            "vouchers, general/subsidiary ledgers, month-end close, "
            "expense-reimbursement review, trial balances, and inventory / "
            "fixed-asset accounting. Give it the source documents or data and "
            "the specific question — it answers in CAS entries and tables."
        ),
        mode="ptc",
        role_prompt_template="roles/finance_accountant.md.j2",
        tools=["execute_code", "filesystem", "bash", "finance", "web_search"],
        max_iterations=15,
        stateful=True,
        sections={
    ...
    "treasury": SubagentDefinition(
        name="treasury",
        description=(
            "Delegate cash and liquidity work to the treasury sub-agent: "
            "30/90-day cash-flow forecasts, bank reconciliation and balance "
            "regulation statements, position monitoring, receipts/payments "
            "planning, and treasury-risk flags (large abnormal flows, "
            "related-party fund occupation)."
        ),
        mode="ptc",
        role_prompt_template="roles/finance_treasury.md.j2",
        tools=["execute_code", "filesystem", "bash", "finance", "web_search"],
        max_iterations=15,
        stateful=True,
        sections={
    ...
    "tax-specialist": SubagentDefinition(
        name="tax-specialist",
        description=(
            "Delegate tax work to the tax specialist sub-agent: VAT / CIT / "
            "surcharge / stamp computations with shown rates, filing-prep "
            "data packages, tax-burden analysis, incentive identification "
            "(small-micro, R&D super-deduction, accelerated depreciation), "
            "and tax-risk flags."
        ),
        mode="ptc",
        role_prompt_template="roles/finance_tax.md.j2",
        tools=["execute_code", "filesystem", "bash", "finance", "web_search"],
        max_iterations=15,
        stateful=True,
        sections={
    ...
    "fp-analyst": SubagentDefinition(
        name="fp-analyst",
        description=(
            "Delegate financial analysis to the FP&A sub-agent: three-statement "
            "trend/structure/ratio analysis (profitability, working capital, "
            "solvency), budget-vs-actual variance, and management "
            "recommendations backed by computed figures and charts."
        ),
        mode="ptc",
        role_prompt_template="roles/finance_fpa.md.j2",
        tools=["execute_code", "filesystem", "bash", "finance", "web_search"],
        max_iterations=20,
        stateful=True,
        sections={
    ...
    "internal-auditor": SubagentDefinition(
        name="internal-auditor",
        description=(
            "Delegate internal-control and audit work to the internal auditor "
            "sub-agent: segregation-of-duties and approval-control reviews, "
            "fraud-signal detection, compliance checks, analytical procedures, "
            "and a graded findings list (high/medium/low) with remediation "
            "priorities. Independent stance — evidence only."
        ),
        mode="ptc",
        role_prompt_template="roles/finance_auditor.md.j2",
        tools=["execute_code", "filesystem", "bash", "finance", "web_search"],
        max_iterations=20,
        stateful=True,
        sections={
