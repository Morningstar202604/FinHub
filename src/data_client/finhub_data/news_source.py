"""News data source backed by finhub-data."""
...
def _normalize_article(raw: dict[str, Any]) -> dict[str, Any]:
    """Map finhub-data article schema → common NewsArticle dict."""
...
class FinHubDataNewsSource:
    """Fetches news from the finhub-data service.
    ...
    async def get_news(
        ...
        async def _fetch(ticker: str) -> list[dict[str, Any]]:
            try:
                ...
                logger.warning(
                    "news.finhub_data.ticker_fetch_failed | ticker=%s",
                    ticker,
                    exc_info=True,
                )
                return []
    ...
    async def get_news_article(
        ...
        try:
            ...
            logger.warning("news.finhub_data.article_lookup_failed", exc_info=True)
        return None

    async def close(self) -> None:
        pass  # lifecycle managed by FinHubDataClient singleton
