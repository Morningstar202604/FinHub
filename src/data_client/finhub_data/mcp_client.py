"""FinHub-data client for MCP servers running in Daytona sandboxes.

Uses file-based OAuth tokens for authentication, with auto-refresh on 401.
This is the sandbox counterpart to :class:`FinHubDataClient` (which uses
...
FINHUB_INTERVAL_MAP: dict[str, str] = {
    "1min": "1/minute",
    "5min": "5/minute",
    "15min": "15/minute",
    "30min": "30/minute",
...
# Token file path. The host injects FINHUB_TOKEN_FILE with the exact path it
...
TOKEN_FILE = Path(
    os.environ.get("FINHUB_TOKEN_FILE", "/home/workspace/_internal/.mcp_tokens.json")
)

# Declared User-Agent for every sandbox→finhub-data request (data + token
...
_USER_AGENT = "finhub-sandbox/1.0"
...
class FinHubMCPClient:
    """Sandbox-side finhub-data client with OAuth token-file auth.
    ...
    async def ensure(self) -> bool:
        ...
        tokens = _load_tokens()
        finhub_url = tokens.get("finhub_data_url") or os.getenv("FINHUB_DATA_URL", "")
        access_token = tokens.get("access_token", "")

        if finhub_url and access_token:
            self._http = httpx.AsyncClient(
                base_url=finhub_url.rstrip("/"),
                headers={
    ...
    def _error_dict(action: str, e: Exception) -> dict:
        ...
        if isinstance(e, httpx.HTTPStatusError):
            ...
            return {"error": f"finhub-data error ({e.response.status_code}): {detail}"}
        return {"error": f"{action} failed ({type(e).__name__})"}

    async def request(self, method: str, url: str, **kwargs: Any) -> httpx.Response:
        """Make a finhub-data request with auto-refresh on 401."""
        if not await self.ensure():
            raise RuntimeError("finhub-data client not initialized")
    ...
    async def fetch_stock_data(
        ...
        """Fetch stock OHLCV from finhub-data with auto-pagination.
        ...
        finhub_interval = FINHUB_INTERVAL_MAP.get(interval_lower)
        if interval_lower in DAILY_INTERVALS:
            finhub_interval = "1/day"
        if not finhub_interval:
            return None
        ...
        multiplier, timespan = finhub_interval.split("/")
        params: dict[str, Any] = {
            "multiplier": multiplier,
            "timespan": timespan,
            "limit": 50000,
        ...
        try:
            ...
            if 400 <= status < 500 and status != 429:
                return {"error": f"finhub-data error ({status}): {detail}"}
            logger.warning("finhub-data %s for %s: %s", status, symbol, detail)
            return None
        except Exception:
            logger.debug("finhub-data fetch failed for %s", symbol, exc_info=True)
            return None
    ...
    async def fetch_options_chain(
        ...
        """
        if not await self.ensure():
            return {"error": "Options data requires finhub-data (not configured)."}
    ...
    async def fetch_options_snapshot(
        ...
        try:
            ...
            return {"count": len(results), "data": results, "source": "finhub-data"}
        except Exception as e:  # noqa: BLE001
    ...
    async def fetch_short_data(
        ...
        if not await self.ensure():
            return {"error": "Short data requires finhub-data (not configured)."}

        result: dict[str, Any] = {"symbol": symbol, "source": "finhub-data"}
