"""finhub-data implementation of MarketDataSource."""
...
def normalize_series(rows: list[dict], *, ref: InstrumentRef, schema: str) -> Series:
    """Normalize finhub-data bars (epoch-ms ``time`` already UTC) to a Series."""
    return build_series(
        rows, ref=ref, schema=schema, publisher="finhub-data",
        ts_of=lambda row: int(row["time"]) if row.get("time") else 0,
    )

# FMP-style interval → finhub-data (timespan, multiplier). Second bars are
...
class FinHubDataSource:
    """Market data source backed by finhub-data REST API."""
    ...
    def __init__(self, client: FinHubDataClient) -> None:
        self.client = client
    ...
    def _index_symbol(symbol: str) -> str:
        """Convert a Yahoo/FMP-style index symbol to finhub-data format."""
    ...
    def _default_dates(
        ...
        """finhub-data requires from/to — supply sensible defaults."""
        if to_date is None:
    ...
    def _normalize(row: dict[str, Any]) -> dict[str, Any]:
        """Normalize a finhub-data bar to the standard OHLCV shape."""
        return {
            "time": row.get("time", 0),
    ...
    async def get_market_status(
        ...
        """Fetch current market status from finhub-data."""
    ...
    def _normalize_snapshot(raw: dict[str, Any], asset_type: str = "stocks") -> dict[str, Any]:
        """Normalize a finhub-data snapshot to the unified snapshot shape."""
