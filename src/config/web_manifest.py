...
Data-only module (stdlib only) — it must never import search.py, langchain, or
provider packages, so the resolve-time gate and write validation can import it
cheaply.

Lives in ``config`` rather than ``tools/web`` because that is what its
consumers are: the LLM resolve-time tier gate (``server.services.llm.config``),
the preferences write validation (``server.app.users``, ``server.app.api_keys``)
and the cost/pricing table (``utils.tracking.infrastructure_costs``). All four
read provider *metadata*; none of them invokes a web capability. Filing it under
``tools`` made the capability layer the owner of a type the application shell
depends on, which is the `server -> tools` inversion tracked by
``scripts/guard/layering_guard.py``. ``tools.web.manifest`` remains as a
re-export shim for the tool-internal callers.
"""
...
_MANIFEST_PATH = Path(__file__).parent / "manifest" / "web_providers.json"
