<p align="center">
  ...
  <br>
  FinHub は、金融市場の理解と投資判断を支援するために作られています。
  <br><br>
...
着想はソフトウェアエンジニアリングにあります。codebase は継続して存在し、各 commit はそれ以前の作業の上に積み上がります。Claude Code や OpenCode のようなコード agent harness が成果を上げたのは、agent が既存の文脈を探索し、過去の作業を土台に進められるようにしたからです。FinHub は同じ発想を投資に持ち込みます。agent に永続的な workspace を与えることで、リサーチは自然に積み上がります。
...
- **Enterprise Finance Team** — 会計・資金・税務・FP&A・内部監査の各専門 subagent を内蔵。記帳、キャッシュフロー予測、税額計算、予実分析、内部監査などの企業財務ワークフローに対応する専門ロールプロンプトを備えています。
...
- **Channel integrations** — Slack、Discord、Feishu、Telegram から FinHub を使えます。スケジュール結果はメール配信もできます。
...
FinHub は、複数の LLM backend を抽象化する provider-agnostic なモデル層で動作します。どのモデルが駆動していても、同じ middleware stack、tools、workflow が利用できます。標準で 2 つのモードを備えています。
...
FinHub は 3 層の data provider hierarchy をサポートします。各層は任意で、上位層が利用できない場合でも、システムは自動的に下位層へフォールバックします。
...
FinHub は credential、code execution、user-supplied secrets に対し、layered security model を適用します。
