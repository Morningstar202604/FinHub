...
export const MessageBubble = memo(function MessageBubble({ message, turnIndex, isTurnTail, feedback, isLoading, hideAvatar, compactToolCalls, isSubagentView, readOnly, allowFiles, isMobile, flashContext }: MessageBubbleProps): React.ReactElement {
  ...
  return (
    ...
    >
      {/* Assistant avatar - shown on the left, Doubao-style brand medallion:
          the logo sits on a soft amber disc so the assistant reads as one
          recognizable identity in every thread. */}
      {isAssistant && !hideAvatar && (
        <div
          className={`flex-shrink-0 rounded-full flex items-center justify-center overflow-hidden ${isMobile ? 'w-7 h-7' : 'w-9 h-9'}`}
          style={{
            backgroundColor: 'var(--color-accent-soft)',
            boxShadow: '0 0 0 1px rgba(233, 149, 74, 0.18), 0 2px 8px rgba(233, 149, 74, 0.10)',
          }}
        >
          <img
            src={logo}
      ...
      <div className={`${isUser ? (isEditing && isMobile ? 'w-full' : 'max-w-[80%]') + ' flex flex-col items-end gap-2' : `w-full min-w-0${isMobile ? '' : ' pt-1'}`}`}>
        ...
        <div
          className={`rounded-2xl ${
            isUser
              ? `${isMobile ? 'px-3 py-2' : 'px-4 py-3'} rounded-tr-md overflow-hidden`
              : `pl-0 pr-0 ${isMobile ? 'pb-2' : 'pb-3'} rounded-tl-md`
          }`}
          style={{
            ...
            borderColor: isUser
              ...
              : 'transparent',
            boxShadow: isUser
              ? '0 1px 3px rgba(0, 0, 0, 0.12), 0 4px 14px rgba(233, 149, 74, 0.08)'
              : 'none',
            color: 'var(--color-text-primary)',
          }}
        >
      ...
      {isUser && !hideAvatar && !(isEditing && isMobile) && (
        <div
          ...
          style={{
            backgroundColor: 'var(--color-accent-soft)',
            boxShadow: '0 0 0 1px rgba(233, 149, 74, 0.14)',
          }}
        >
          {avatarUrl ? (
