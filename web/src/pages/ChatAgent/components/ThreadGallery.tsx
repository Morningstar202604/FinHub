...
import { ArrowLeft, Folder, FileText, Zap, Archive, ChartCandlestick, TrendingUp, Target, Newspaper } from 'lucide-react';
...
function ThreadGallery({ workspaceId, onBack, onThreadSelect }: ThreadGalleryProps) {
  ...
  // seeds the composer so the user can edit before sending.
  const SUGGESTIONS: {
    titleKey: string;
    descKey: string;
    promptKey: string;
    ...
    {
      titleKey: 'thread.suggestMarketTitle',
      descKey: 'thread.suggestMarketDesc',
      promptKey: 'thread.suggestMarketPrompt',
      icon: ChartCandlestick,
      tileVar: 'var(--color-tile-blue)',
    },
    {
      titleKey: 'thread.suggestReportTitle',
      descKey: 'thread.suggestReportDesc',
      promptKey: 'thread.suggestReportPrompt',
      icon: TrendingUp,
      tileVar: 'var(--color-tile-gold)',
    },
    {
      titleKey: 'thread.suggestScreenTitle',
      descKey: 'thread.suggestScreenDesc',
      promptKey: 'thread.suggestScreenPrompt',
      icon: Target,
      tileVar: 'var(--color-tile-green)',
    },
    {
      titleKey: 'thread.suggestNewsTitle',
      descKey: 'thread.suggestNewsDesc',
      promptKey: 'thread.suggestNewsPrompt',
      icon: Newspaper,
      tileVar: 'var(--color-tile-purple)',
    },
  ];
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage: 'var(--color-bg-atmosphere)',
        ...
        <div ref={scrollContainerRef} className="flex-1 flex flex-col min-h-0 w-full px-4 overflow-auto">
          <div className="w-full max-w-[768px] mx-auto flex flex-col gap-8">
            ...
            <div className="w-full flex flex-col items-center mt-2 md:mt-[8vh] enter-fade-up enter-fade-up-d1">
              <div
                className="flex items-center justify-center transition-colors cursor-pointer w-16 h-16 rounded-2xl border"
                style={{
              ...
              >
                {isFlash ? (
                  <Zap className="w-7 h-7" style={{ color: 'var(--color-accent-primary)' }} />
                ) : (
                  <img src={iconComputer} alt="Workspace" className="w-9 h-9" />
                )}
              </div>
              <h1
            ...
            {/* Suggestion cards — Doubao/通义-style capability prompts */}
                ...
                {SUGGESTIONS.map((s) => {
                  ...
                  return (
                    <button
                      key={s.titleKey}
                      type="button"
                      onClick={() => handleSuggestion(t(s.promptKey))}
                      className="group flex items-start gap-3 rounded-2xl p-4 text-left border transition-all duration-200 hover:-translate-y-0.5 hover:brightness-[1.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1"
                      style={{
                        borderColor: 'var(--color-border-muted)',
                        backgroundColor: 'var(--color-bg-card)',
                        boxShadow: 'var(--shadow-card)',
                    ...
                    >
                      <span
                        aria-hidden="true"
                        className="flex items-center justify-center w-10 h-10 rounded-xl flex-shrink-0 text-white shadow-md"
                        style={{ background: s.tileVar }}
                      >
                      ...
                      <span className="flex flex-col gap-1 min-w-0">
                        ...
                        <span className="text-xs leading-relaxed line-clamp-2" style={{ color: 'var(--color-text-tertiary)' }}>
                          {t(s.descKey)}
                        </span>
                      </span>
                    </button>
            ...
            {!isFlash && <div className="w-full enter-fade-up enter-fade-up-d4">
              <div
            ...
            {/* Threads Section */}
            <div className="w-full flex flex-col gap-4 pb-8 enter-fade-up enter-fade-up-d5">
